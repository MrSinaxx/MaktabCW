...
def login_user():
    ...
    user = DBManager.read_user_by_username(username)
...
def display_inbox():
    ...
    messages = DBManager.get_inbox_messages(user_id)
    print("Inbox:")
...
def display_outbox():
    user_id = current_user._id
    messages = DBManager.get_outbox_messages(user_id)
    print("Outbox:")
    for message in messages:
...
def send_message():
    ...
    recipient = DBManager.read_user_by_username(to_user)
    if recipient:
        message = Message(from_user=from_user, to_user=recipient._id, subject=subject, content=content)
        DBManager.create(message)
        print("Message sent successfully!")
    else:
        print("Recipient not found. Please check the username.")
...
def main_menu():
    menu_data = {
        ...
        "children": [
            ...
            {
                "name": "Inbox",
                "action": display_inbox,
            },
            {
                "name": "Outbox",
                "action": display_outbox,
            },
            {
                "name": "Send Message",
                "action": send_message,
    ...
    menu = generate_menu_from_dict(menu_data)
    menu()
